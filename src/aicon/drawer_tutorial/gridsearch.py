...
def get_all_jobs():
    ...
    jobs = []
    for combination in itertools.product(*(spec["sweep_values"] for spec in sweep_specs)):
        trial_params = copy.deepcopy(base_params)
        connection_params = {}
        job_groups = []
        job_names = []
        for spec, (sweep_label, sweep_value) in zip(sweep_specs, combination):
        ...
        jobs.append(
            {
                ...
                "param_name": ",".join(spec["param_name"] for spec in sweep_specs),
                "standard_value": None,
...
def get_sweep_values(standard_value, param_name: str = None):
    """Return sweep values. For threshold-like params return three sensible values (low, standard, high).

    Detection uses param_name substrings like 'threshold', 'uncertainty', 'existence', 'likelihood'.
    """
    name = (param_name or "").lower()
    is_threshold = any(k in name for k in ("threshold", "existence", "likelihood"))

    if is_threshold:
        # If value is probability-like (0..1) keep bounds inside [0,1]
        try:
            s = float(standard_value)
        except Exception:
            s = standard_value

        if isinstance(s, (int, float)) and 0.0 <= s <= 1.0:
            low = max(0.0, s * 0.2)
            high = min(1.0, s * 5 if s > 0 else 0.5)
        else:
            low = s * 0.2
            high = s * 5.0

        return [("low", low), ("standard", standard_value), ("high", high)]

    # default multiplicative sweep
    return [("x0.2", 0.2 * standard_value), ("standard", standard_value), ("x5", 5.0 * standard_value)]
...
def save_job_metadata(job: dict, results: list, out_dir: str = "results_gridsearch", job_index: int = None) -> tuple:
...
def main(job_index:int):
    ...
    try:
        json_path, csv_path = save_job_metadata(job, results, out_dir="results_gridsearch", job_index=job_index)
